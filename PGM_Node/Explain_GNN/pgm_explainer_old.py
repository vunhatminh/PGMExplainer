...
class Node_Explainer:
    ...
    def explain(self, node_idx, num_samples = 100, top_node = None, p_threshold = 0.05, pred_threshold = 0.1):
        ...
        nA = n_hops_A(self.A, self.num_layers)
        node_idx_new, sub_A, sub_X, sub_L, neighbors = self.extract_n_hops_neighbors(nA,node_idx)
        
        ind_sub_to_ori = dict(zip(range(len(neighbors)), neighbors))
        ind_ori_to_sub = dict(zip(neighbors,range(len(neighbors))))
        ...
        pgm_nodes = []
        if top_node == None:
            pgm_nodes = [node for node in dependent_neighbors]
        else:
    ...
    def test(self, node_list, num_samples = 10, top_node = None, pred_threshold = 0.1):        
        print("X dim: ", self.X.shape)
        print("A dim: ", self.A.shape)
        time.sleep(10)
